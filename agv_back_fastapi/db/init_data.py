...
def init_process():
    datas = [(0, '到电梯', fake.paragraph(nb_sentences=3, variable_nb_sentences=True, ext_word_list=None)),
             (1, '载物', fake.paragraph(nb_sentences=3, variable_nb_sentences=True, ext_word_list=None)),
        ...
        try:
            for i in range(init_item_num):
                ...
                item = Items(name=f'商品{i + 1}',
                             ...
                             user_id=1)
                process_queue_ls = choose_process(min_process_num, max_process_num, process_num - 1)
                is_go_to_lift = fake.boolean()
                if is_go_to_lift:
                    process_queue_ls.append(0)
    ...
    with get_session() as session:
        try:
            for i in range(1):
                ...
                battery = Battery(
                    **dict(name="电池", topicName="/battery", topicType="communicate_with_stm32/BatteryInfo",
                           currentBattery=11))
